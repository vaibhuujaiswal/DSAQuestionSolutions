...
public:
    ...
    Node* post_order(int pre[], int size) {
        ...
        int currentIndex = 1;
        createTree(pre, size, root, INT_MIN, INT_MAX,currentIndex);
        return dummy;
    }

    void createTree(int pre[], int size, Node* root, int minVal, int maxVal, int& currentIndex) {
        if (currentIndex == size) return;
        if (pre[currentIndex] > maxVal) {
            return;
        }
        // Left subtree
        if (pre[currentIndex] < root->data) {
            root->left = newNode(pre[currentIndex]);
            currentIndex++;
            createTree(pre, size, root->left, minVal, root->data,currentIndex);
        }
if (pre[currentIndex] > maxVal) {
            return;
        }
        // Right subtree
        if (currentIndex < size && pre[currentIndex] > root->data) {
            root->right = newNode(pre[currentIndex]);
            currentIndex++;
            createTree(pre, size, root->right, root->data, maxVal,currentIndex);
        }
    }
};
